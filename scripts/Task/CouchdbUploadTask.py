...
class UploaderWorker:
    ...
    def __store_ids__(self, log_id, couch_id, parent_id, rev, material_id):
        ...
        sql = "insert into json_couch_ids " \
              "(log_id, couch_id, parent_id, rev, material_id) values" \
              " (%d, '%s', '%s' '%s', '%s')" % \
              (log_id, couch_id, parent_id, rev, material_id)
...
class Uploader:
    def __init__(self, sql_server, user, passwd, sql_db, couch_server, couch_port):
        ...
        self.passwd = passwd
        self.charset = "utf8"
        self.importor = UploaderWorker(couch_server, couch_port)
    ...
    def run(self):
        ...
        for row in formator_record_fetch_cursor:
            ...
            if 0 != self.importor.batch_import(json_content):
